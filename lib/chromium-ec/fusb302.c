...
static void detect_cc_pin_source_manual(int port, enum tcpc_cc_voltage_status* cc1_lvl, enum tcpc_cc_voltage_status* cc2_lvl) {
...
static void detect_cc_pin_sink(int port, enum tcpc_cc_voltage_status* cc1, enum tcpc_cc_voltage_status* cc2) {
    int reg;
    int orig_meas_cc1;
    int orig_meas_cc2;
    int bc_lvl_cc1;
...
static int fusb302_tcpm_init(int port) {
	 ...
	 */
    tcpc_read(port, TCPC_REG_STATUS0, &reg);
    if(reg & TCPC_REG_STATUS0_VBUSOK) usb_charger_vbus_change(port, 1);
#endif
...
static int fusb302_tcpm_get_cc(int port, enum tcpc_cc_voltage_status* cc1, enum tcpc_cc_voltage_status* cc2) {
    if(state[port].pulling_up) {
        /* Source mode? */
...
static int fusb302_tcpm_set_cc(int port, int pull) {
    ...
    case TYPEC_CC_RP:
        ...
        reg &=
            ~(TCPC_REG_SWITCHES0_CC2_PU_EN | TCPC_REG_SWITCHES0_CC1_PU_EN | TCPC_REG_SWITCHES0_CC1_PD_EN | TCPC_REG_SWITCHES0_CC2_PD_EN |
              TCPC_REG_SWITCHES0_VCONN_CC1 | TCPC_REG_SWITCHES0_VCONN_CC2);

        reg |= TCPC_REG_SWITCHES0_CC1_PU_EN | TCPC_REG_SWITCHES0_CC2_PU_EN;

        if(state[port].vconn_enabled) reg |= state[port].cc_polarity ? TCPC_REG_SWITCHES0_VCONN_CC1 : TCPC_REG_SWITCHES0_VCONN_CC2;
...
__maybe_unused static int fusb302_tcpm_decode_sop_prime_enable(int port, bool enable) {
    int reg;

    if(tcpc_read(port, TCPC_REG_CONTROL1, &reg)) return EC_ERROR_UNKNOWN;
    ...
    if(enable) {
        ...
        if(IS_ENABLED(CONFIG_USB_PD_DECODE_SOP)) {
            if(state[port].rx_enable) {
                if(fusb302_tcpm_decode_sop_prime_enable(port, true)) return EC_ERROR_UNKNOWN;
            }
        }
    } else {
        tcpc_read(port, TCPC_REG_SWITCHES0, &reg);
        ...
        if(IS_ENABLED(CONFIG_USB_PD_DECODE_SOP)) {
            if(state[port].rx_enable) {
                if(fusb302_tcpm_decode_sop_prime_enable(port, false)) return EC_ERROR_UNKNOWN;
            }
        }
    }
...
static int fusb302_tcpm_set_msg_header(int port, int power_role, int data_role) {
    ...
    if(power_role) reg |= TCPC_REG_SWITCHES1_POWERROLE;
    if(data_role) reg |= TCPC_REG_SWITCHES1_DATAROLE;
...
static int fusb302_tcpm_set_rx_enable(int port, int enable) {
    ...
    if(enable) {
        ...
        if(!tcpc_read(port, TCPC_REG_MASK, &reg)) tcpc_write(port, TCPC_REG_MASK, reg | TCPC_REG_MASK_BC_LVL);
...
static int fusb302_rx_fifo_is_empty(int port) {
    ...
    return (!tcpc_read(port, TCPC_REG_STATUS1, &reg)) && (reg & TCPC_REG_STATUS1_RX_EMPTY);
}
...
static int fusb302_tcpm_get_message_raw(int port, uint32_t* payload, int* head) {
...
#ifdef CONFIG_USB_PD_DECODE_SOP
    {
        ...
        if(tcpc_read(port, TCPC_REG_STATUS1, &reg)) return EC_ERROR_UNKNOWN;
...
static int fusb302_tcpm_transmit(int port, enum tcpci_msg_type type, uint16_t header, const uint32_t* data) {
    /*
...
void fusb302_tcpc_alert(int port) {
	 ...
	 */
    if(state[port].rx_enable) interrupt &= ~TCPC_REG_INTERRUPT_BC_LVL;
    ...
    if(interrupt & TCPC_REG_INTERRUPT_VBUSOK) {
        ...
        usb_charger_vbus_change(port, fusb302_tcpm_check_vbus_level(port, VBUS_PRESENT));
#else
        if(!fusb302_tcpm_check_vbus_level(port, VBUS_PRESENT)) pd_vbus_low(port);
#endif
    ...
    if(interrupta & TCPC_REG_INTERRUPTA_HARDRESET) {
        ...
        /* bring FUSB302 out of reset */
        fusb302_pd_reset(port);
        task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_RX_HARD_RESET);
    }
...
static int fusb302_set_toggle_mode(int port, int mode) {
    ...
    rv = i2c_read8(tcpc_config[port].i2c_info.port, tcpc_config[port].i2c_info.addr_flags, TCPC_REG_CONTROL2, &reg);
    if(rv) return rv;

    reg &= ~TCPC_REG_CONTROL2_MODE_MASK;
    reg |= mode << TCPC_REG_CONTROL2_MODE_POS;
    return i2c_write8(tcpc_config[port].i2c_info.port, tcpc_config[port].i2c_info.addr_flags, TCPC_REG_CONTROL2, reg);
}
...
static int fusb302_tcpm_enter_low_power_mode(int port) {
	 ...
	 * - start toggling
	 */
    rv = i2c_write8(tcpc_config[port].i2c_info.port, tcpc_config[port].i2c_info.addr_flags, TCPC_REG_POWER, TCPC_REG_POWER_PWR_LOW);
    if(rv) return rv;
    ...
    case PD_DRP_TOGGLE_OFF:
        ...
        break;
    case PD_DRP_FREEZE:
        mode = pd_get_power_role(port) == PD_ROLE_SINK ? TCPC_REG_CONTROL2_MODE_UFP : TCPC_REG_CONTROL2_MODE_DFP;
        break;
...
int fusb302_get_vbus_voltage(int port, int* vbus) {
	 ...
	 */
    for(i = 5; i >= 0; i--) {
        if(fusb302_compare_mdac(port, mdac | BIT(i))) mdac |= BIT(i);
    }
