...
Стартовое значение `tx_spec_rev = 01b` (PD 2.0) — самый безопасный default.

При первом RX-сообщении от партнёра с `SpecRev = 10b`:
- Партнёр заявляет PD 3.x.
- Обновляем `tx_spec_rev = 10b` — наши исходящие будут с PD 3.0 header.
- Это применяется через `SWITCHES1.SPEC_REV = 01b → 10b` (для auto-GoodCRC) и в нашем header construction.
