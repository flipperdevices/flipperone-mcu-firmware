...
static inline void start_condition_program_init(PIO pio, uint sm, uint offset,
                                                uint pin);
static inline void stop_condition_program_init(PIO pio, uint sm, uint offset,
                                               uint pin);
static inline void read_byte_program_init(PIO pio, uint sm, uint offset,
                                          uint pin);
static inline void write_byte_program_init(PIO pio, uint sm, uint offset,
                                           uint pin);
...
void i2c_multi_init(PIO pio, uint pin) {
  ...
  start_condition_program_init(pio, i2c_multi->sm_start,
                               i2c_multi->offset_start, pin);
  i2c_multi->offset_stop = pio_add_program(pio, &stop_condition_program);
  i2c_multi->sm_stop = pio_claim_unused_sm(pio, true);
  stop_condition_program_init(pio, i2c_multi->sm_stop, i2c_multi->offset_stop,
                              pin);
  ...
  write_byte_program_init(pio, i2c_multi->sm_write, i2c_multi->offset_write,
                          pin);
  pio_sm_put(pio, i2c_multi->sm_read,
             (((uint32_t)do_ack_program_instructions[1]) << 16) |
                 do_ack_program_instructions[0]);
  pio_sm_put(pio, i2c_multi->sm_read,
             (((uint32_t)do_ack_program_instructions[3]) << 16) |
                 do_ack_program_instructions[2]);
...
static inline void byte_handler_pio(void) {
  ...
  if (i2c_multi->status != I2C_WRITE) {
    received = transpond_byte(
        pio_sm_get_blocking(i2c_multi->pio, i2c_multi->sm_read) >>
  ...
  if (i2c_multi->status == I2C_IDLE) {
    if (!i2c_multi_is_address_enabled(received >> 1)) {
      ...
      i2c_multi->bytes_count = 0;
      pio_sm_put(
          i2c_multi->pio, i2c_multi->sm_read,
          (((uint32_t)do_ack_program_instructions[10] + i2c_multi->offset_read)
           << 16) |
  ...
  if (i2c_multi->status == I2C_READ) {
    pio_sm_put(i2c_multi->pio, i2c_multi->sm_read,
               (((uint32_t)do_ack_program_instructions[5]) << 16) |
                   do_ack_program_instructions[4]);
    pio_sm_put(
        i2c_multi->pio, i2c_multi->sm_read,
        (((uint32_t)do_ack_program_instructions[7] + i2c_multi->offset_read)
         << 16) |
  ...
  if (i2c_multi->status == I2C_WRITE && is_address) {
    ...
    pio_sm_put(
        i2c_multi->pio, i2c_multi->sm_read,
        (((uint32_t)do_ack_program_instructions[8] + i2c_multi->offset_read)
         << 16) |
            do_ack_program_instructions[6]);
    pio_sm_put(i2c_multi->pio, i2c_multi->sm_read,
               (((uint32_t)do_ack_program_instructions[1]) << 16) |
    ...
    pio_sm_put(i2c_multi->pio, i2c_multi->sm_write,
               (((uint32_t)wait_ack_program_instructions[1]) << 16) |
                   wait_ack_program_instructions[0]);
    pio_sm_put(i2c_multi->pio, i2c_multi->sm_write,
               (((uint32_t)wait_ack_program_instructions[3]) << 16) |
                   wait_ack_program_instructions[2]);
  }

  if (i2c_multi->status == I2C_WRITE && !is_address) {
    if (i2c_multi->length == -1 ||
        (i2c_multi->length > 0 &&
         i2c_multi->bytes_count < i2c_multi->length + 1)) {
      uint8_t value = 0;
      if (i2c_multi->buffer) {
        value = transpond_byte(*i2c_multi->buffer);
        i2c_multi->buffer++;
      ...
      pio_sm_put(i2c_multi->pio, i2c_multi->sm_write,
                 (((uint32_t)wait_ack_program_instructions[5]) << 16) |
                     wait_ack_program_instructions[4]);
      pio_sm_put(i2c_multi->pio, i2c_multi->sm_write,
                 (((uint32_t)wait_ack_program_instructions[7] +
                   i2c_multi->offset_write)
                  << 16) |
