...
#define DMESG_APP_LOG_BUFFER_SIZE (1024 * 8)
...
static void dmesg_app_log_tx_callback(const uint8_t* buffer, size_t size, void* context) {
    ...
    circular_buffer_write(instance->log_buffer, buffer, size, true);
}
...
int32_t dmesg_app(void* p) {
    ...
    dmesg_app_head->log_buffer = circular_buffer_alloc(DMESG_APP_LOG_BUFFER_SIZE);
