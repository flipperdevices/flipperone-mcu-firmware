        ...
        if(furi_mutex_acquire(instance->mutex, FuriWaitForever) == FuriStatusOk) {
            ...
            for(size_t y = 0; y < instance->height; y += 4) {
                ...
                for(size_t x = 0; x < instance->width; x += 2) {
                    ...
                    uint8_t braille_byte = (p1 << 0) | (p2 << 1) | (p3 << 2) | (p4 << 3) | (p5 << 4) | (p6 << 5) | (p7 << 6) | (p8 << 7);
...
static void cli_command_screen_emulate_click(FuriPubSub* input_pubsub, InputKey key) {
    InputEvent event = {.sequence_source = INPUT_SEQUENCE_SOURCE_SOFTWARE, .sequence_counter = 0, .key = key, .type = InputTypePress};
...
static void cli_command_screen_click_callback(PipeSide* pipe, void* context) {
    ...
    while(pipe_bytes_available(pipe) > 0) {
        ...
        if(ansi_res.is_done) {
            ...
            case 'W':
            case 'w':
                emulated_key = InputKeyUp;
                break;
            case CliKeyDown:
            case 'S':
            case 's':
                emulated_key = InputKeyDown;
                break;
            case CliKeyLeft:
            case 'A':
            case 'a':
                emulated_key = InputKeyLeft;
                break;
            case CliKeyRight:
            case 'D':
            case 'd':
                emulated_key = InputKeyRight;
                break;
            ...
            case 'O':
            case 'o':
                emulated_key = InputKeyOk;
                break;

            case '\t':
            case 'X':
            case 'x':
                emulated_key = InputKeySw;
                break;
            case 'P':
            case 'p':
                emulated_key = InputKeyPtt;
                break;

            case '1':
                emulated_key = InputKey1;
                break;
            case '2':
                emulated_key = InputKey2;
                break;
            case '3':
                emulated_key = InputKeyPower;
                break;
            case '4':
                emulated_key = InputKey4;
                break;
            case '5':
                emulated_key = InputKey5;
                break;
