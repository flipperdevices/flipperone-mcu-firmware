...
static void complex_layout(App* app) {
    furi_assert(app);
    ComplexAppState* state = (ComplexAppState*)app->state;
    int32_t selected_document_index = state->selected_document_index;
    DocumentArray documents = state->documents;
...
static bool complex_input(App* app, const GuiTestMessage* message) {
    furi_assert(app);
    furi_assert(message);
    ...
    case GuiTestMessageTypeInputEvent: {
        ...
        if(event.type == InputTypePress && event.key == InputKeyOk) {
            state->selected_document_index++;
            if(state->selected_document_index >= state->documents.length) {
                state->selected_document_index = 0;
            }
        }
    } break;
...
static void complex_scroll(App* app) {
    furi_assert(app);
    ComplexAppState* state = (ComplexAppState*)app->state;
...
App app_complex = {
    ...
    .input = complex_input,
    .render = complex_layout,
    .scroll = complex_scroll,
};
