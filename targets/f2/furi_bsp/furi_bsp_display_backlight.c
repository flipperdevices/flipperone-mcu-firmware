...
void furi_bsp_display_backlight_set_brightness(uint8_t brightness) {
    ...
    if(brightness == 0) {
        if(handle_backlight.backlight_pwm) {
            ...
            furi_hal_pwm_deinit(handle_backlight.backlight_pwm);
            handle_backlight.backlight_pwm = NULL;
        }
    } else {
        ...
        if(!handle_backlight.backlight_pwm) {
            ...
            handle_backlight.backlight_pwm = furi_hal_pwm_init(&gpio_display_backlight_en, BACKLIGHT_PWM_RESOLUTION, BACKLIGHT_PWM_FREQ_HZ, false);
            furi_hal_pwm_set_duty_cycle(handle_backlight.backlight_pwm, max_value);
            furi_delay_us(500);
            furi_hal_pwm_set_duty_cycle(handle_backlight.backlight_pwm, 2);
